...
func (s *BundleAPI) CallBundle(ctx context.Context, args CallBundleArgs) (map[string]interface{}, error) {
	...
	for k, tx := range txs {
		...
		expectedTx := args.Txs[k].ToTransaction()
		state.SetTxContext(expectedTx.Hash(), k)
		...
		//	//})
		//
		//	rules := s.chain.Config().Rules(header.Number, header.Difficulty.Cmp(common.Big0) == 0)
		//	state.Prepare(rules, tx.From(), coinbase, tx.To(), vm.ActivePrecompiles(rules), tx.AccessList())
		//	receipt, result, err := core.ApplyTransactionWithResult(s.b.ChainConfig(), s.chain, &coinbase, gp, state, header, tx, expectedTx, &header.GasUsed, vmconfig)
		if err != nil {
